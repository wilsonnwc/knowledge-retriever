...
- **IMMEDIATE NEXT:** Extract the service layer (`notes_store`/`embed`/`skills_store`/`events_store` per `system/learning-os-plan.md` Section B) — then wire Search/Chat to real search using the Q1/Q2/Q3 decisions above, closing the UI backlog. MCP server (Learning OS Phase 1) comes after that, not before.
