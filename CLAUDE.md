...
- **Session 25 (2026-08-26/27) — design only, no code:** Planned the evolution of this project into a **learning operating system** — `ai-chief-of-staff`'s daily intake ranked against knowledge state + skill gaps instead of a static topic hierarchy, with a main orchestrator agent, an MCP server, and continuous eval (top-down invariants + bottom-up error analysis). Full plan with every decision and trade-off: **`system/learning-os-plan.md`** (readable 3-column version: https://claude.ai/code/artifact/67345e9a-61a6-4b96-82b1-bb27b3dadf65). **Blocking open question before phase 0 can be scoped:** one shared eval harness across both projects, or one per project?
- **IMMEDIATE NEXT:** Either continue the existing UI backlog — (1) paste-formatting-assist on import (ties to the existing "ingestion agent" roadmap item), (2) URL-based ingest with LLM summarization, (3) Search/Chat is still 100% mock, blocked on 3 open UX questions from Session 19 — or start the Learning OS plan at phase 0 (repair eval debt + extract the service layer). Note the **eval-repair session is now a prerequisite** under the new plan, not a deferred item: 20 of 28 locked eval queries in `system/evaluation/test_queries.json` reference filenames the Session 22 consolidation deleted/renamed, and the planned Novelty agent's value rests entirely on retrieval quality.
