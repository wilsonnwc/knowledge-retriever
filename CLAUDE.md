...
- **Phase 6 (Layer 3 taste — write-time related notes):** Complete. `--suggest-related <note-path>` compares a note against every embedded note, ranks by best-matching-chunk-per-note, groups results by shared project. Wired as an ask-first step in the bulk-import workflow, offered only when the new note has a `projects:` tag. Deliberately covers only one slice of "living knowledge base" (finding-to-finding connections) — not staleness detection or categorization. See `system/session-log.md` Session 13, including a real distance-metric bug found and fixed (Chroma defaults to L2 distance, not cosine — the code had assumed cosine).
...
- **Next:** A read-time extension of related-notes (ask after chat answers a question, on the top-ranked retrieved note) is scoped but blocked on a small refactor decision — see Session 13 open questions. Interview-defense drill still deferred (Session 12 + 13 items).
