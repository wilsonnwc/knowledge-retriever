...
- **Session 27 (2026-08-30) — Phase 0 in progress:** Started the "repair eval debt" half of Phase 0. Found and fixed three real bugs in `search_notes()` (keyword baseline): `.trash` notes were still being searched/embedded, whole-file match counting structurally favored long consolidated files over short focused ones (fixed by scoring per-section via the existing chunker), and there was no IDF weighting (a hand-rolled stopword list was only a partial fix — replaced the scorer entirely with `rank_bm25`/BM25, the standard algorithm, after web research confirmed IDF is current best practice over stopword lists). **Precision@5: 54% (broken baseline) → 75% (21/28)**, with the remaining 7 failures cleanly diagnosed as 2 genuine already-documented synonymy limits (no fix needed) and 5 stale test labels from the Session 22 consolidation (fix identified, pending explicit go-ahead since `test_queries.json` is locked — see `system/session-log.md` Session 27 for the exact diagnosis). **Uncommitted:** `requirements.txt` (+`rank_bm25`), `scripts/chat.py`, `scripts/chunking.py`.
- **IMMEDIATE NEXT:** Apply the 5 pending `expected_source` label corrections to `test_queries.json`, re-run the eval to confirm it lands at/near the historic 82% ceiling, commit the Session 27 changes — then finish Phase 0 by extracting the service layer (`notes_store`/`embed`/`skills_store`/`events_store`). After Phase 0: Phase 1 (MCP server, read-only, stdio) per `system/learning-os-plan.md` Section G. The pre-existing UI backlog (paste-formatting-assist, URL ingest, Search/Chat still mock) remains parked behind the Learning OS build sequence, not abandoned.
