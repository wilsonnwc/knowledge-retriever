...
- **IMMEDIATE NEXT (undecided — see Session 32/33 options):** either (a) MCP server (Learning OS Phase 1 — the already-decided next roadmap step per Session 29's sequencing call, now unblocked), or (b) two small deferred polish items from Search/Chat: surface the freshness tripwire via Flask too (e.g. `/api/health`), and before/after sentence-highlighting for source snippets if the plain-passage version feels unfocused in real use.
