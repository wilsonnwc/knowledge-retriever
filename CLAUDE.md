...
- **IMMEDIATE NEXT (before anything else):** (1) Add `OPENAI_API_KEY` to `.env` — still missing as of end of Session 21. (2) Actually try the backend — user has not yet run it. Start with: `cd knowledge-retriever && source .venv/bin/activate && python3 backend/app.py`, then in another terminal `curl http://localhost:5000/api/health` and `curl http://localhost:5000/api/notes`. Do this before starting any new work.
