...
- **Phase 7 eval infrastructure (Session 17):** Built real eval cases and LLM-as-judge evaluation script for `research_goal()` coverage step. Five test cases seeded from Session 15 bugs; first run: 3/5 passing (60%, all rule-based checks passed, LLM-as-judge needs parsing refinement). Acceptance bar grounded in production-like standards, not arbitrary thresholds. `auto_narrow=True` parameter added to unblock non-interactive eval runs.
- **Next:** Investigate why rg_001/rg_002 LLM-as-judge checks failed (likely parsing fragility); decide whether to refine parsing or fall back to rule-based + manual spot-check approach. Build case-by-case decision tree for "eval fails: now what?" (refine test, fix implementation, or lower bar?). Unresolved: roadmap item 6 (lightweight UI)'s build-order sequencing — user wants to describe a third option beyond "design shell first" vs. "build everything together," not yet captured. Interview-defense drill still deferred (Session 12–17 items — now includes explaining keyword-search retirement, truncation bug, three research-goal bugs, temperature tuning, and eval infrastructure philosophy). 5 deferred "Why this matters" TODOs remain across the two Kindle imports. Formal eval script (keyword search baseline) not re-run since the two Kindle imports were added. `claude-brain/skills-to-learn.md` tracks all of this centrally — check it at the start of a session for the full open list across projects.
