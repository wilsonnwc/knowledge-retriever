...
- **Next:** User tests with 3 goal candidates using checklist template, returns findings. Prioritize UX fixes based on testing patterns, then build lightweight UI (roadmap item 6). Deferred: interview-defense drill; 5 "Why this matters" TODOs on Kindle imports; keyword baseline re-run; Layer 3's other two slices (categorization/staleness).
