...
- **IMMEDIATE NEXT:** Wire the Flask backend + React UI to `retrieval_service` using the Session 19 UX decisions (real Read view, templated sources elaboration, no new API cost), closing the UI backlog. MCP server (Learning OS Phase 1) comes after that, not before.
