...
- **IMMEDIATE NEXT:** Start scoping/building the Projects UI (backend: expose `load_projects`/`new_project`/`archive_project` via Flask, matching `notes_store.py`'s existing error-handling conventions; frontend: a simple list/create/archive view, replacing the sidebar's disabled placeholder). Not yet started — this is the first task for the next session. After that: MCP server (Learning OS Phase 1). Small deferred items from Search/Chat still open: surface the freshness tripwire via Flask (e.g. `/api/health`), and before/after sentence-highlighting for source snippets if the plain-passage version feels unfocused in real use.
