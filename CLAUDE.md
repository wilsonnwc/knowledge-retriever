...
- **IMMEDIATE NEXT:** Manual browser re-verification of the streaming + scroll fixes (servers already running: Flask `:5050`, React dev server `:3000`). Then: surface the freshness tripwire via Flask too (e.g. `/api/health`), and consider before/after sentence-highlighting for source snippets if the plain-passage version feels unfocused in real use. MCP server (Learning OS Phase 1) comes after that, not before.
