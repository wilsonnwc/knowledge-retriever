...
- **Next (Session 18):** Design comprehensive eval checklist for `research_goal()` using the three saved UX design frameworks + PM-level trust/error/interaction principles. Checklist should inform both feature defaults and fallback behaviors. Then: decide lightweight UI build-sequencing approach (user to propose third option beyond "design shell first" vs. "build everything together"). Deferred: interview-defense drill (now includes eval methodology + architecture mismatch diagnosis); 5 "Why this matters" TODOs on Kindle imports; keyword baseline re-run; Layer 3's other two slices (categorization/staleness). `claude-brain/skills-to-learn.md` tracks all centrally.
