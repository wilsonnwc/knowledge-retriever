...
- **IMMEDIATE NEXT:** None of these are started — pick order at will: (1) delete-note functionality, (2) a dedicated read view (currently only truncated preview or full-edit mode), (3) paste-formatting-assist on import (ties to the existing "ingestion agent" roadmap item), (4) URL-based ingest with LLM summarization, (5) wiring the Import wizard's Confirm step to the backend (confirmed via live testing it currently does nothing), (6) Search/Chat is still 100% mock, blocked on 3 open UX questions from Session 19 (Go-to-article behavior, how sources-elaboration text should be generated + eval discipline, its API cost). Separately: an **eval-repair session** (deliberately deferred until all UI work is done) — 20 of 28 locked eval queries in `system/evaluation/test_queries.json` reference filenames the Session 22 consolidation deleted/renamed.
