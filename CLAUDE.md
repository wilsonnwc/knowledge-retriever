...
- **Next:** Build real eval cases for the `research_goal()` coverage step, hands-on across all three approaches discussed (rule-based/exact-match, LLM-as-judge, manual eval) — see Session 16 log for the agreed case format and starter cases. Also unresolved: roadmap item 6 (lightweight UI)'s build-order sequencing — user wants to describe a third option beyond "design shell first" vs. "build everything together," not yet captured. Interview-defense drill still deferred (Session 12–16 items — now includes explaining the keyword-search retirement, the truncation bug, the three research-goal bugs, and temperature tuning). 5 deferred "Why this matters" TODOs remain across the two Kindle imports. Formal eval script not re-run since the two Kindle imports were added. `claude-brain/skills-to-learn.md` tracks all of this centrally — check it at the start of a session for the full open list across projects.
