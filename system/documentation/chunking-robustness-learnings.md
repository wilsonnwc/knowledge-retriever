...
**Why:** this project doubles as interview preparation for a role where the product (LeapSpace) mostly *can't* control source content structure — researchers upload arbitrary PDFs, and a plausible future direction involves connecting to researchers' existing tools (see `system/documentation/job-ad-reference.md`'s Notion/Obsidian/MCP discussion). The read-time robustness path is the closer analog to that actual constraint, so it's the more directly relevant thing to have built and be able to discuss. The ingestion-agent idea is recorded as a future roadmap item in `CLAUDE.md` for when the project is used for genuine ongoing personal note-taking, where — unlike LeapSpace — the author *does* control the source.
